...
const actions: ActionTree<IUserTicketsState, TRootState> = {
  async [GET_DATA] ({ state, commit }) {
    const { data } = await this.service.services.estate.getCommerce({
      ...state.filters,
  ...
  async [UPLOAD_IMAGE] (_, { id, payload }) {
    await this.service.services.estate.uploadCommerceImage(id, payload);
  },

  async [DELETE_IMAGE] (_, id) {
    await this.service.services.estate.deleteCommerceImage(id);
  },

  async [DELETE_DOCUMENT] (_, id) {
    await this.service.services.estate.deleteCommerceDocument(id);
  },

  async [GET_DETAILS] ({ commit }, id) {
    const { data } = await this.service.services.estate.getCommerceDetails(id);

    commit(SET_DETAILS, data);
  },

  async [UPDATE_DETAILS] (_, { id, payload }) {
    await this.service.services.estate.updateCommerceDetails(id, payload);
  },

  async [GET_REFERENCES] ({ commit }) {
    const { data } = await this.service.services.estate.getCommerceReferences();
...
const getters: GetterTree<IUserTicketsState, TRootState> = {
  ...
  statuses (state) {
    return state.references.status.map((item: any) => ({
      value: item.id,
      label: item.name
    }));
  },
