...
const actions: ActionTree<IUserTicketsState, TRootState> = {
  async [GET_DATA] ({ state, commit }) {
    const { data } = await this.service.services.improvement.getFlowerData({
      ...state.filters,
  ...
  async [UPLOAD_IMAGE] (_, { id, payload }) {
    await this.service.services.improvement.uploadFlowerImage(id, payload);
  },

  async [DELETE_IMAGE] (_, id) {
    await this.service.services.improvement.deleteFlowerImage(id);
  },

  async [DELETE_DOCUMENT] (_, id) {
    await this.service.services.improvement.deleteFlowerDocument(id);
  },

  async [GET_DETAILS] ({ commit }, id) {
    const { data } = await this.service.services.improvement.getFlowerDetails(id);

    commit(SET_DETAILS, data);
  },

  async [UPDATE_DETAILS] (_, { id, payload }) {
    await this.service.services.improvement.updateFlowerDetails(id, payload);
  },

  async [GET_REFERENCES] ({ commit }) {
    const { data } = await this.service.services.improvement.getFlowerReferences();
...
const flower: Module<IUserTicketsState, TRootState> = {
  namespaced: true,
  state,
  mutations,
  actions,
