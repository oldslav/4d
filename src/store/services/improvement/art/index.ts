...
const actions: ActionTree<IUserTicketsState, TRootState> = {
  async [GET_DATA] ({ state, commit }) {
    const { data } = await this.service.services.improvement.getArtData({
      ...state.filters,
  ...
  async [UPLOAD_IMAGE] (_, { id, payload }) {
    await this.service.services.improvement.uploadArtImage(id, payload);
  },

  async [DELETE_IMAGE] (_, id) {
    await this.service.services.improvement.deleteArtImage(id);
  },

  async [DELETE_DOCUMENT] (_, id) {
    await this.service.services.improvement.deleteArtDocument(id);
  },

  async [GET_DETAILS] ({ commit }, id) {
    const { data } = await this.service.services.improvement.getArtDetails(id);

    commit(SET_DETAILS, data);
  },

  async [UPDATE_DETAILS] (_, { id, payload }) {
    await this.service.services.improvement.updateArtDetails(id, payload);
  },

  async [GET_REFERENCES] ({ commit }) {
    const { data } = await this.service.services.improvement.getArtReferences();
...
const art: Module<IUserTicketsState, TRootState> = {
  namespaced: true,
  state,
  mutations,
  actions,
