...
const actions: ActionTree<IUserTicketsState, TRootState> = {
  async [GET_DATA] ({ state, commit }) {
    const { data } = await this.service.services.improvement.getTreesData({
      ...state.filters,
  ...
  async [UPLOAD_IMAGE] (_, { id, payload }) {
    await this.service.services.improvement.uploadTreesImage(id, payload);
  },

  async [DELETE_IMAGE] (_, id) {
    await this.service.services.improvement.deleteTreesImage(id);
  },

  async [DELETE_DOCUMENT] (_, id) {
    await this.service.services.improvement.deleteTreesDocument(id);
  },

  async [GET_DETAILS] ({ commit }, id) {
    const { data } = await this.service.services.improvement.getTreesDetails(id);

    commit(SET_DETAILS, data);
  },

  async [UPDATE_DETAILS] (_, { id, payload }) {
    await this.service.services.improvement.updateTreesDetails(id, payload);
  },

  async [GET_REFERENCES] ({ commit }) {
    const { data } = await this.service.services.improvement.getTreesReferences();
...
const getters: GetterTree<IUserTicketsState, TRootState> = {
  plants (state) {
    return state.references.plant.map((item: any) => ({
      value: item.id,
      label: item.name
    }));
  },
...
const trees: Module<IUserTicketsState, TRootState> = {
  namespaced: true,
  state,
  mutations,
  actions,
