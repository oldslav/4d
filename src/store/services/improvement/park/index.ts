...
const actions: ActionTree<IUserTicketsState, TRootState> = {
  async [GET_DATA] ({ state, commit }) {
    const { data } = await this.service.services.improvement.getParkData({
      ...state.filters,
  ...
  async [UPLOAD_IMAGE] (_, { id, payload }) {
    await this.service.services.improvement.uploadParkImage(id, payload);
  },

  async [DELETE_IMAGE] (_, id) {
    await this.service.services.improvement.deleteParkImage(id);
  },

  async [DELETE_DOCUMENT] (_, id) {
    await this.service.services.improvement.deleteParkDocument(id);
  },

  async [GET_DETAILS] ({ commit }, id) {
    const { data } = await this.service.services.improvement.getParkDetails(id);

    commit(SET_DETAILS, data);
  },

  async [UPDATE_DETAILS] (_, { id, payload }) {
    await this.service.services.improvement.updateParkDetails(id, payload);
  },

  async [GET_REFERENCES] ({ commit }) {
    const { data } = await this.service.services.improvement.getParkReferences();
...
const park: Module<IUserTicketsState, TRootState> = {
  namespaced: true,
  state,
  mutations,
  actions,
