...
const actions: ActionTree<IUserTicketsState, TRootState> = {
  async [GET_DATA] ({ state, commit }) {
    const { data } = await this.service.services.improvement.getSquareData({
      ...state.filters,
  ...
  async [UPLOAD_IMAGE] (_, { id, payload }) {
    await this.service.services.improvement.uploadSquareImage(id, payload);
  },

  async [DELETE_IMAGE] (_, id) {
    await this.service.services.improvement.deleteSquareImage(id);
  },

  async [DELETE_DOCUMENT] (_, id) {
    await this.service.services.improvement.deleteSquareDocument(id);
  },

  async [GET_DETAILS] ({ commit }, id) {
    const { data } = await this.service.services.improvement.getSquareDetails(id);

    commit(SET_DETAILS, data);
  },

  async [UPDATE_DETAILS] (_, { id, payload }) {
    await this.service.services.improvement.updateSquareDetails(id, payload);
  },

  async [GET_REFERENCES] ({ commit }) {
    const { data } = await this.service.services.improvement.getSquareReferences();
...
const square: Module<IUserTicketsState, TRootState> = {
  namespaced: true,
  state,
  mutations,
  actions,
