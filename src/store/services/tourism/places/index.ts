...
const actions: ActionTree<IUserTicketsState, TRootState> = {
  async [GET_DATA] ({ state, commit }) {
    const { data } = await this.service.services.tourism.getPlacesData({
      ...state.filters,
  ...
  async [UPLOAD_IMAGE] (_, { id, payload }) {
    await this.service.services.tourism.uploadPlacesImage(id, payload);
  },

  async [DELETE_IMAGE] (_, id) {
    await this.service.services.tourism.deletePlacesImage(id);
  },

  async [DELETE_DOCUMENT] (_, id) {
    await this.service.services.tourism.deletePlacesDocument(id);
  },

  async [GET_DETAILS] ({ commit }, id) {
    const { data } = await this.service.services.tourism.getPlacesDetails(id);

    commit(SET_DETAILS, data);
  },

  async [UPDATE_DETAILS] (_, { id, payload }) {
    await this.service.services.tourism.updatePlacesDetails(id, payload);
  },

  async [GET_REFERENCES] ({ commit }) {
    const { data } = await this.service.services.tourism.getPlacesReferences();
