...
const actions: ActionTree<IUserTicketsState, TRootState> = {
  async [GET_DATA] ({ state, commit }) {
    const { data } = await this.service.services.transport.getParkingData({
      ...state.filters,
  ...
  async [UPLOAD_IMAGE] (_, { id, payload }) {
    await this.service.services.transport.uploadParkingImage(id, payload);
  },

  async [DELETE_IMAGE] (_, id) {
    await this.service.services.transport.deleteParkingImage(id);
  },

  async [DELETE_DOCUMENT] (_, id) {
    await this.service.services.transport.deleteParkingDocument(id);
  },

  async [GET_DETAILS] ({ commit }, id) {
    const { data } = await this.service.services.transport.getParkingDetails(id);

    commit(SET_DETAILS, data);
  },

  async [UPDATE_DETAILS] (_, { id, payload }) {
    await this.service.services.transport.updateParkingDetails(id, payload);
  },

  async [GET_REFERENCES] ({ commit }) {
    const { data } = await this.service.services.transport.getParkingReferences();
