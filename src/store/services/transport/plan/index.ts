...
const actions: ActionTree<IUserTicketsState, TRootState> = {
  async [GET_DATA] ({ state, commit }) {
    const { data } = await this.service.services.transport.getPlanData({
      ...state.filters,
  ...
  async [UPLOAD_IMAGE] (_, { id, payload }) {
    await this.service.services.transport.uploadPlanImage(id, payload);
  },

  async [DELETE_IMAGE] (_, id) {
    await this.service.services.transport.deletePlanImage(id);
  },

  async [DELETE_DOCUMENT] (_, id) {
    await this.service.services.transport.deletePlanDocument(id);
  },

  async [GET_DETAILS] ({ commit }, id) {
    const { data } = await this.service.services.transport.getPlanDetails(id);

    commit(SET_DETAILS, data);
  },

  async [UPDATE_DETAILS] (_, { id, payload }) {
    await this.service.services.transport.updatePlanDetails(id, payload);
  },

  async [GET_REFERENCES] ({ commit }) {
    const { data } = await this.service.services.transport.getPlanReferences();
...
const plan: Module<IUserTicketsState, TRootState> = {
  namespaced: true,
  state,
  mutations,
  actions,
