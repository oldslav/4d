...
const actions: ActionTree<IUserTicketsState, TRootState> = {
  async [GET_DATA] ({ state, commit }) {
    const { data } = await this.service.services.transport.getHighwayData({
      ...state.filters,
  ...
  async [UPLOAD_IMAGE] (_, { id, payload }) {
    await this.service.services.transport.uploadHighwayImage(id, payload);
  },

  async [DELETE_IMAGE] (_, id) {
    await this.service.services.transport.deleteHighwayImage(id);
  },

  async [DELETE_DOCUMENT] (_, id) {
    await this.service.services.transport.deleteHighwayDocument(id);
  },

  async [GET_DETAILS] ({ commit }, id) {
    const { data } = await this.service.services.transport.getHighwayDetails(id);

    commit(SET_DETAILS, data);
  },

  async [UPDATE_DETAILS] (_, { id, payload }) {
    await this.service.services.transport.updateHighwayDetails(id, payload);
  },

  async [GET_REFERENCES] ({ commit }) {
    const { data } = await this.service.services.transport.getHighwayReferences();
...
const highway: Module<IUserTicketsState, TRootState> = {
  namespaced: true,
  state,
  mutations,
  actions,
