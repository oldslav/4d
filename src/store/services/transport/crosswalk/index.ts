...
const actions: ActionTree<IUserTicketsState, TRootState> = {
  async [GET_DATA] ({ state, commit }) {
    const { data } = await this.service.services.transport.getCrosswalkData({
      ...state.filters,
  ...
  async [UPLOAD_IMAGE] (_, { id, payload }) {
    await this.service.services.transport.uploadCrosswalkImage(id, payload);
  },

  async [DELETE_IMAGE] (_, id) {
    await this.service.services.transport.deleteCrosswalkImage(id);
  },

  async [DELETE_DOCUMENT] (_, id) {
    await this.service.services.transport.deleteCrosswalkDocument(id);
  },

  async [GET_DETAILS] ({ commit }, id) {
    const { data } = await this.service.services.transport.getCrosswalkDetails(id);

    commit(SET_DETAILS, data);
  },

  async [UPDATE_DETAILS] (_, { id, payload }) {
    await this.service.services.transport.updateCrosswalkDetails(id, payload);
  },

  async [GET_REFERENCES] ({ commit }) {
    const { data } = await this.service.services.transport.getCrosswalkReferences();
...
const crosswalk: Module<IUserTicketsState, TRootState> = {
  namespaced: true,
  state,
  mutations,
  actions,
