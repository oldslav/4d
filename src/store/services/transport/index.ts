...
const actions: ActionTree<IUserTicketsState, TRootState> = {
  async [GET_DATA] ({ state, commit }) {
    const { data } = await this.service.services.transport.getData({
      ...state.filters,
  ...
  async [UPLOAD_IMAGE] (_, { id, payload }) {
    await this.service.services.transport.uploadImage(id, payload);
  },

  async [DELETE_IMAGE] (_, id) {
    await this.service.services.transport.deleteImage(id);
  },

  async [DELETE_DOCUMENT] (_, id) {
    await this.service.services.transport.deleteDocument(id);
  },

  async [GET_DETAILS] ({ commit }, id) {
    const { data } = await this.service.services.transport.getDetails(id);

    commit(SET_DETAILS, data);
  },

  async [UPDATE_DETAILS] (_, { id, payload }) {
    await this.service.services.transport.updateDetails(id, payload);
  },

  async [GET_REFERENCES] ({ commit }) {
    const { data } = await this.service.services.transport.getReferences();
