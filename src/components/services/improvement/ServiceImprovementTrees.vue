...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState("services/improvement/trees", {
        filters: state => state.filters
      }),

      ...mapGetters("services/improvement/trees", [
        "tablePagination",
        "tableData",
        "plants"
      ]),

      ...mapFields("services/improvement/trees", {
        fields: ["plantId", "query"],
        base: "filters",
      ...
      isLoading () {
        return this.$store.state.wait[`services/improvement/trees/${ GET_DATA }`];
      }
    },
    methods: {
      moment,
      ...
      ...mapActions("services/improvement/trees", {
        GET_DATA,
        GET_REFERENCES
      }),
      ...
      toDetails (id) {
        this.$router.push({
          name: "gis-services-improvement-trees-details",
          params: {
            id
          }
        });
