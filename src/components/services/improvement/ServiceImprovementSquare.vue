      ...
      <template #body="props">
        <q-tr :props="props" class="cursor-pointer" @click.stopPropagation="toDetails(props.row.id)">
          ...
          <q-td key="name" :props="props">
            <span>
              {{ formatEmptyString(props.row.name) }}
            </span>
          </q-td>
          <q-td key="category" :props="props">
            <span>
              {{ formatEmptyString(props.row.category && props.row.category.name) }}
            </span>
          </q-td>
          <q-td key="installedYear" :props="props" auto-width>
            <span>
              {{ props.row.installedYear }}
            </span>
          </q-td>
          <q-td key="updatedAt" :props="props" auto-width>
            <span>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState("services/improvement/square", {
        filters: state => state.filters
      }),

      ...mapGetters("services/improvement/square", [
        "tablePagination",
        "tableData",
        "types"
      ]),

      ...mapFields("services/improvement/square", {
        fields: ["typeId", "query"],
        base: "filters",
      ...
      isLoading () {
        return this.$store.state.wait[`services/improvement/square/${ GET_DATA }`];
      }
    },
    methods: {
      moment,
      ...
      ...mapActions("services/improvement/square", {
        GET_DATA,
        GET_REFERENCES
      }),
      ...
      toDetails (id) {
        this.$router.push({
          name: "gis-services-improvement-square-details",
          params: {
            id
          }
        });
