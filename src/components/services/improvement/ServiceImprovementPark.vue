      ...
      <template #body="props">
        <q-tr :props="props" class="cursor-pointer" @click.stopPropagation="toDetails(props.row.id)">
          ...
          <q-td key="material" :props="props">
            <span>
              {{ formatEmptyString(props.row.pavingType && props.row.pavingType.name) }}
            </span>
          </q-td>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState("services/improvement/park", {
        filters: state => state.filters
      }),

      ...mapGetters("services/improvement/park", [
        "tablePagination",
        "tableData",
        "types"
      ]),

      ...mapFields("services/improvement/park", {
        fields: ["typeId", "query"],
        base: "filters",
      ...
      isLoading () {
        return this.$store.state.wait[`services/improvement/park/${ GET_DATA }`];
      }
    },
    methods: {
      moment,
      ...
      ...mapActions("services/improvement/park", {
        GET_DATA,
        GET_REFERENCES
      }),
      ...
      toDetails (id) {
        this.$router.push({
          name: "gis-services-improvement-park-details",
          params: {
            id
          }
        });
