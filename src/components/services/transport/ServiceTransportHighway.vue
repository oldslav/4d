...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState("services/transport/highway", {
        filters: state => state.filters
      }),

      ...mapGetters("services/transport/highway", [
        "tablePagination",
        "tableData",
        "infrastructureTypes"
      ]),

      ...mapFields("services/transport/highway", {
        fields: ["typeId", "query"],
        base: "filters",
      ...
      isLoading () {
        return this.$store.state.wait[`services/transport/highway/${ GET_DATA }`];
      }
    },
    methods: {
      moment,
      ...
      ...mapActions("services/transport/highway", {
        GET_DATA,
        GET_REFERENCES
      }),
      ...
      toDetails (id) {
        this.$router.push({
          name: "gis-services-transport-highway-details",
          params: {
            id
          }
        });
