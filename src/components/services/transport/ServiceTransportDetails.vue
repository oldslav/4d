...
<script>
  ...
  import transport from "../../../store/services/transport";
  ...
  export default {
    ...
    preFetch ({ store, currentRoute }) {
      ...
      return Promise.all([
        store.dispatch(`services/transport/${ GET_REFERENCES }`),
        store.dispatch(`services/transport/${ GET_DETAILS }`, currentRoute.params.id)
      ]);
    },
    async created () {
    ...
    computed: {
      ...
      isLoading () {
        return this.$store.state.wait[`services/transport/${ GET_DETAILS }`] ||
          this.$store.state.wait[`services/transport/${ DELETE_IMAGE }`];
      },

      imageUploadUrl () {
        return process.env.SERVER_API_HOST + `/api/v1/map/trobject/gis/${ this.current && this.current.id }/image`;
      },

      documentsUploadUrl () {
        return process.env.SERVER_API_HOST + `/api/v1/map/trobject/gis/${ this.current && this.current.id }/documents`;
      },
    ...
    methods: {
      ...
      ...mapMutations("services/transport", [
        SET_DETAILS_EMPTY
      ]),
