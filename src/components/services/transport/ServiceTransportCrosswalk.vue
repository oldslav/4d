...
<script>
  ...
  export default {
    name: "ServiceTransportCrosswalk",
    ...
    computed: {
      ...mapState("services/transport/crosswalk", {
        filters: state => state.filters
      }),

      ...mapGetters("services/transport/crosswalk", [
        "tablePagination",
        "tableData",
        "infrastructureTypes"
      ]),

      ...mapFields("services/transport/crosswalk", {
        fields: ["typeId", "query"],
        base: "filters",
      ...
      isLoading () {
        return this.$store.state.wait[`services/transport/crosswalk/${ GET_DATA }`];
      }
    },
    methods: {
      moment,
      ...
      ...mapActions("services/transport/crosswalk", {
        GET_DATA,
        GET_REFERENCES
      }),
      ...
      toDetails (id) {
        this.$router.push({
          name: "gis-services-transport-crosswalk-details",
          params: {
            id
          }
        });
