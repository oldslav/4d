...
<script>
  ...
  export default {
    name: "ServiceTransportTrobjects",
    ...
    computed: {
      ...mapState("services/transport/trobjects", {
        filters: state => state.filters
      }),

      ...mapGetters("services/transport/trobjects", [
        "tablePagination",
        "tableData",
        "infrastructureTypes"
      ]),

      ...mapFields("services/transport/trobjects", {
        fields: ["typeId", "query"],
        base: "filters",
      ...
      isLoading () {
        return this.$store.state.wait[`services/transport/trobjects/${ GET_DATA }`];
      }
    },
    methods: {
      moment,

      ...mapActions("services/transport/trobjects", {
        GET_DATA,
        GET_REFERENCES
      }),
      ...
      toDetails (id) {
        this.$router.push({
          name: "gis-services-transport-trobjects-details",
          params: {
            id
          }
        });
