...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState("services/transport/plan", {
        filters: state => state.filters
      }),

      ...mapGetters("services/transport/plan", [
        "tablePagination",
        "tableData",
        "categories"
      ]),

      ...mapFields("services/transport/plan", {
      ...
      isLoading () {
        return this.$store.state.wait[`services/transport/plan/${ GET_DATA }`];
      }
    },
    methods: {
      moment,
      ...
      ...mapActions("services/transport/plan", {
        GET_DATA,
        GET_REFERENCES
      }),
      ...
      toDetails (id) {
        this.$router.push({
          name: "gis-services-transport-plan-details",
          params: {
            id
          }
        });
