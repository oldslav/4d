...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState("services/transport/parking", {
        filters: state => state.filters
      }),

      ...mapGetters("services/transport/parking", [
        "tablePagination",
        "tableData",
        "infrastructureTypes"
      ]),

      ...mapFields("services/transport/parking", {
        fields: ["typeId", "query"],
        base: "filters",
      ...
      isLoading () {
        return this.$store.state.wait[`services/transport/parking/${ GET_DATA }`];
      }
    },
    methods: {
      moment,

      ...mapActions("services/transport/parking", {
        GET_DATA,
        GET_REFERENCES
      }),
      ...
      toDetails (id) {
        this.$router.push({
          name: "gis-services-transport-parking-details",
          params: {
            id
          }
        });
