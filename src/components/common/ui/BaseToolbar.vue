  ...
  q-toolbar
    template(v-if="isMobile")
      BaseTabs(v-model="tab")
        q-route-tab(:to="{ name: 'main' }" name="main" :label="$t('entity.main')")
        q-route-tab(:to="{ name: 'playground' }" name="playground" :label="$t('entity.playground')")
        q-route-tab(:to="{ name: 'profile' }" name="profile"  :label="$t('entity.profile')")
        q-route-tab(:to="{ name: 'error-page' }" name="error-page" label="Ошибка")
    ...
    template(v-else)
      BaseTabs(v-model="tab")
        ...
        q-route-tab(:to="{ name: 'error-page' }" name="error-page" label="Ошибка")
    q-space
    q-btn(round dense icon="o_account_circle" color="primary")
      q-menu
        q-list(dense)
